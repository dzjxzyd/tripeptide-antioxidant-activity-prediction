...
#Int64Index([252, 271, 275, 280, 299, 339, 364, 411, 522, 678, 717, 753, 758,
#            760, 774, 802, 993],
#           dtype='int64')

type(selected_features_rfe)
selected_features_rfe[0]
...
pd.DataFrame(sum).to_csv('1111.csv')
pd.DataFrame(model.coef_).to_csv('2222.csv')


### connected them
#X_rfe is the new feature for model
name[a[0]]
#####
X_new = X_rfe
X_new.shape
...
for kk in range(20):
    ...
    #
    model_SVR_linear = SVR(kernel='rbf',degree = 3,C= 5).fit(X_important_train, y_train)
...
summary.to_csv('bootstrap_REF_linear regression_.csv')
