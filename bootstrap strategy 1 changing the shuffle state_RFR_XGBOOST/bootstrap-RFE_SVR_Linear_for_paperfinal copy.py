...
##### note: there are tow different SVE kernel you can try out
model = SVR(kernel='linear')
model.fit(X_train,y_train)
model.score(X_test,y_test)#0.9840465475972281
...
model.fit(X_rfe,y)
model.score(X_rfe,y)#0.9157480418783905

temp = pd.Series(rfe.support_,index = cols)
selected_features_rfe = temp[temp==True].index
print(selected_features_rfe)
# Int64Index([14,  120,  214,  292,  299,  319,  410,  459,  486,  661,  717,
#             814,  828,  938,  975,  991, 1011],
#             dtype='int64')
...
print(sum)
#['CHAM820102', 'NAKH920101', 'RICJ880114', 'PARS000102', 'CEDJ970105', 'GEOR030105', 'GEIM800106', 'NAKH900108', 'PALJ810116', 'GEOR030105', 'CHOP780215', 'OOBM850102', 'PALJ810116', 'WERD780104', 'PARS000101', 'MITS020101', 'DIGM050101']


pd.DataFrame(sum).to_csv('RFE_SVR_name.csv')
pd.DataFrame(model.coef_).to_csv('RFE_SVR_featureimportance.csv')
...
for kk in range(20):
    ...
    ##### cross validation
    rf = xgb.XGBRegressor( max_depth=2,n_estimators=202,reg_alpha= 5,  reg_lambda = 50)
    model = rf
    ...
    import xgboost as xgb
    from sklearn.metrics import r2_score, mean_squared_error, explained_variance_score
    rf = xgb.XGBRegressor( booster = 'gblinear',learning_rate=0.3,n_estimators=204,reg_alpha= 0.1,  reg_lambda = 0)
    model = rf
    model.fit(X_important_train, y_train)
    train_score = model.score(X_important_train, y_train)
    train_score
    ...
    ### 3. random forest\
    from sklearn.ensemble import RandomForestRegressor
    rfr = RandomForestRegressor(bootstrap=True, n_estimators=85,  max_depth=2,random_state = 0)
    model = rfr
    model.fit(X_important_train, y_train)
    train_score = model.score(X_important_train, y_train)
    train_score
    ...
    est = make_pipeline(QuantileTransformer(),
                        MLPRegressor(hidden_layer_sizes=(17,17),
                                        activation = 'tanh',
                                        solver= 'adam' ,
    ...
    #
    model_SVR_linear = SVR(kernel='rbf',degree = 3,C= 8).fit(X_important_train, y_train)
    ...
    # 8. svr_linear
    model_SVR_linear = SVR(kernel='linear',C= 20).fit(X_important_train, y_train)
    model = model_SVR_linear
    model.fit(X_important_train, y_train)
    train_score = model.score(X_important_train, y_train)
    train_score
    ...
    # lassi
    model_Lassco = Lasso(alpha=0.1,tol = 1e-3)
    model = model_Lassco
    model.fit(X_important_train, y_train)
    train_score = model.score(X_important_train, y_train)
    train_score
    ...
    # 10. Ridge
    model_ridge = Ridge(alpha=80,tol = 1e-1)
    model = model_ridge
    model.fit(X_important_train, y_train)
    train_score = model.score(X_important_train, y_train)
    train_score
    ...
    kr=KernelRidge(alpha = 80,degree=5).fit(X_important_train, y_train)
    model = kr
    model.fit(X_important_train, y_train)
    train_score = model.score(X_important_train, y_train)
    train_score
    ...
    huber = HuberRegressor(epsilon = 10,max_iter = 1000,alpha = 0.1).fit(X_important_train, y_train)
    model = huber
    model.fit(X_important_train, y_train)
    train_score = model.score(X_important_train, y_train)
    train_score
    ...
    regr = BaggingRegressor(n_estimators=200,max_features=17, random_state=0).fit(X_important_train, y_train)
    model = regr
    model.fit(X_important_train, y_train)
    train_score = model.score(X_important_train, y_train)
    train_score
...
summary.to_csv('bootstrap_REF_SVR_linear _.csv')
